package com.leohacker.leetcode.LongestValidParentheses;

/**
 * 基本的思路是每次遇到右括号，就开始处理前面未处理的左括号。
 *
 * 考虑()这种情况，右括号的出现就吃掉一个左括号，当前长度是2。如果是(())这种情况，在出现第一个右括号的时候，待处理的数据是(2。
    ...
    public int longestValidParentheses(String s) {
        ...
        for (int i = 0; i < len; i++) {
            ...
            if ( c == '(') {
                ...
                currentLength = 0;
            } else if ( index == 0 ) {  // right parentheses.
                ...
                // since the orphan parentheses interrupt the continuation, we reset the current length.
                currentLength = 0;
            } else {    // index point to the next empty slot.
